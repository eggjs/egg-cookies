...
class Cookies {
   ...
   */
  get(name, opts = {}) {
    const { encrypt, signed } = encryptOrSigned(opts);
    ...
    let value = match[1];
    if (!encrypt && !signed) return value;
...
function encryptOrSigned(opts = {}) {
  ...
  if (opts.encrypt) return { encrypt: true, signed: false };
  if (opts.signed !== false) return { encrypt: false, signed: true };
  return opts;
}
